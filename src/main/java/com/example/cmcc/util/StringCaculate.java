...
public class StringCaculate {
    ...
    private void caculate() {
        ...
        switch (ope) {
            // 如果是加号或者减号，则
            case '+':
                result = a.add(b).setScale(0, BigDecimal.ROUND_HALF_UP);
                // 将操作结果放入操作数栈
//                DebugLog.e(">>>>>>>>>>>>>" + result);
                numbers.push(result);
                break;
            case '-':
                // 将操作结果放入操作数栈
                result = a.subtract(b).setScale(0, BigDecimal.ROUND_HALF_UP);
//                DebugLog.e(">>>>>>>>>>>>>" + result);
                numbers.push(result);
                break;
            case '*':
                result = a.multiply(b).setScale(0, BigDecimal.ROUND_HALF_UP);
//                DebugLog.e(">>>>>>>>>>>>>" + result);
                // 将操作结果放入操作数栈
                numbers.push(result);
                break;
            case '/':
                result = a.divide(b).setScale(2, BigDecimal.ROUND_HALF_UP);// 将操作结果放入操作数栈
//                DebugLog.e(">>>>>>>>>>>>>" + result);
                numbers.push(result);
                break;
        }
